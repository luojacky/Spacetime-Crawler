...
from datamodel.search.Luoj9Joseo1_datamodel import Luoj9Joseo1Link
...
@Deleter(Luoj9Joseo1Link)
@Getter(Luoj9Joseo1Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(Luoj9Joseo1Link)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(Luoj9Joseo1Link, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_Luoj9Joseo1".format(CrawlerFrame.app_id),
